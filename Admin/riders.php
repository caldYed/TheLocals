...
$ongoing_orders = $conn->query("SELECT * FROM orders WHERE rider_id=$rider_id AND status!='Delivered' ORDER BY order_date ASC");

// Fetch ongoing Preorders (not delivered)
$ongoing_preorders = $conn->query("SELECT * FROM preorders WHERE rider_id=$rider_id AND status!='Delivered' ORDER BY created_at ASC");
?>
...
<div class="container">
<h2>Ongoing Deliveries for <?php echo htmlspecialchars($rider_name); ?></h2>
