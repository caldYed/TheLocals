...
// ====== Total sales ======
$order_sales = $conn->query("SELECT COUNT(*) AS total_orders, SUM(total) AS revenue_orders 
                             FROM orders 
                             WHERE status='Delivered' AND order_date >= '$period_sql'");
$order_data = $order_sales->fetch_assoc();

$preorder_sales = $conn->query("SELECT COUNT(*) AS total_preorders, SUM(total) AS revenue_preorders 
                                FROM preorders 
                                WHERE status='Delivered' AND created_at >= '$period_sql'");
$preorder_data = $preorder_sales->fetch_assoc();

$total_revenue = ($order_data['revenue_orders'] ?? 0) + ($preorder_data['revenue_preorders'] ?? 0);

// ====== Product-level sales ======
$order_products = $conn->query("SELECT oi.name AS product_name, SUM(oi.quantity) AS quantity
    FROM order_items oi
    JOIN orders o ON oi.order_id=o.order_id
    WHERE o.status='Delivered' AND o.order_date >= '$period_sql'
    GROUP BY oi.name
    ORDER BY quantity DESC");

$preorder_products = $conn->query("SELECT product_name, SUM(quantity) AS quantity
    FROM preorders
    WHERE status='Delivered' AND created_at >= '$period_sql'
...
$colors_palette = ['#FF6384','#36A2EB','#FFCE56','#4BC0C0','#9966FF','#FF9F40','#8AFF33','#FF33F6','#33FFF3','#FF3333'];
...
list($order_labels, $order_quantities, $order_colors) = prepare_chart_data($order_products, $colors_palette);
list($preorder_labels, $preorder_quantities, $preorder_colors) = prepare_chart_data($preorder_products, $colors_palette);
?>
...
.container { max-width:1200px; margin:auto; background:#fff; padding:20px; border-radius:10px; box-shadow:0 4px 10px rgba(0,0,0,0.1);}
h1 { text-align:center; margin-bottom:20px;}
.summary { display:flex; justify-content:space-around; margin-bottom:20px; flex-wrap:wrap; }
.card { background:#F4C430; padding:15px; border-radius:10px; text-align:center; width:30%; box-shadow:0 2px 6px rgba(0,0,0,0.1); margin-bottom:10px;}
.card h2 { margin:10px 0; font-size:16px; }
a.button { display:inline-block; margin:5px; padding:6px 12px; background:#007bff; color:white; text-decoration:none; border-radius:6px; font-size:14px;}
a.button.active { background:#28a745; }
a.button:hover { background:#0069d9; }
.charts-row { display:flex; justify-content:center; flex-wrap:wrap; margin-bottom:20px; }
.chart-container { width:350px; margin:10px; text-align:center; }
...
<a href="index.php" class="button" style="background:#28a745; margin-top:10px;">Back to Dashboard</a>
        ...
        options: {
            ...
            onClick:(evt,item)=>{
                if(item.length>0){
                    ...
                    alert("Product: "+data.labels[index]+"\nQuantity Sold: "+data.datasets[0].data[index]);
                }
            }
        }
    });
}
createPieChart(document.getElementById('ordersPie').getContext('2d'), ordersData);
createPieChart(document.getElementById('preordersPie').getContext('2d'), preordersData);
</script>
