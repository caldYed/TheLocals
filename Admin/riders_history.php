...
a.button { padding:8px 15px; text-decoration:none; border-radius:6px; color:#fff; font-weight:bold; margin-bottom:20px; display:inline-block; }
a.home { background:#17a2b8; }
a.home:hover { background:#138496; }
a.profile { background:#6c757d; margin-left:10px; }
a.profile:hover { background:#5a6268; }
</style>
</head>
<body>
...
<?php if($riders->num_rows > 0): ?>
    <?php while($rider = $riders->fetch_assoc()): ?>
        <div style="margin-top:20px;">
            ...
            <!-- View Profile button -->
            <a href="rider_profile.php?rider_id=<?php echo $rider['rider_id']; ?>" class="button profile">View Profile</a>
            ...
            <td><?php echo htmlspecialchars($order['address']).', '.htmlspecialchars($order['city']); ?></td>
